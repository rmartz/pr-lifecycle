...
- [Lifecycle routing design (v2)](lifecycle-routing-design.md) — the routing
  extension's goals and decisions (CI gate, merge-conflict and base-health
  states, approval carry-over, auto-update).
...
- [Bot-PR eligibility](bot-eligibility.md) — which bot PRs count as approved
  ...
  never eligible.
- [UAT gate](uat-gate.md) — the opt-in `uat` check-run: the UAT requirement
  read from the head-bound verdict, the human-only overrides and their actor
  check, and how arming waits for it.
