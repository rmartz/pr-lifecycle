...
import type { RestCommit, RestIssueEvent, RestPull, RestReview } from './rest-payloads.js';
...
export function createHttpClient(options: HttpClientOptions): GitHubClient {
  ...
  return {
    ...
    async listLabelEvents(pr): Promise<LabelEventData[]> {
          ...
          ? [
              {
                ...
                // A deleted actor can't be verified, so it's never trusted.
                type: event.actor === null ? 'Bot' : toActorType(event.actor.type),
